...
Three internal routes, identical across processes:
  ...
  GET /internal/health/ready  -> 200 when PG + Redis answer within 500ms each,
...
async def _check_postgres(db_dsn: str) -> tuple[bool, str]:
    ...
    try:
        conn = await asyncio.wait_for(asyncpg.connect(dsn), _READY_TIMEOUT_S)
        try:
            await asyncio.wait_for(conn.execute("SELECT 1"), _READY_TIMEOUT_S)
        finally:
            await conn.close()
...
async def _check_redis(redis_url: str) -> tuple[bool, str]:
    ...
    try:
        pong = await asyncio.wait_for(client.ping(), _READY_TIMEOUT_S)
...
async def check_readiness(settings: BaseProcessSettings) -> tuple[bool, dict[str, str]]:
    ...
    return (pg_ok and redis_ok), {"db": pg_detail, "redis": redis_detail}
...
def _ready_handler(settings: BaseProcessSettings) -> Handler:
    async def ready(request: Request) -> Response:
        ...
        return JSONResponse(
            {"status": "ready" if ok else "not_ready", "dependencies": dependencies},
...
def mount_health(app: Starlette, settings: BaseProcessSettings) -> None:
    """Register the three internal routes on a Starlette or FastAPI app."""
    app.add_route("/internal/health/live", _live, methods=["GET"])
    app.add_route("/internal/health/ready", _ready_handler(settings), methods=["GET"])
    app.add_route("/internal/metrics", _metrics, methods=["GET"])


def build_health_app(settings: BaseProcessSettings) -> Starlette:
    """A Starlette app exposing the three internal routes (non-api processes)."""
    app = Starlette()
    mount_health(app, settings)
    return app


def run_asgi(app: Starlette, port: int) -> None:
    """Serve an ASGI app until SIGTERM/SIGINT (uvicorn owns graceful shutdown)."""
    uvicorn.run(app, host=_BIND_HOST, port=port, log_config=None, access_log=False)
